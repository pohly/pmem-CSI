...
- [PMEM-CSI operator API conversion webhook](#pmem-csi-operator-api-conversion-webhook)
...
If your cluster supports managing the operators using the [Operator Life Manager](https://olm.operatorframework.io/),
then it is recommended to install the PMEM-CSI operator from the [OperatorHub](https://operatorhub.io/operator/pmem-csi-operator).
Otherwise, follow below steps to install the operator manually:
...
| controllerResources | [ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.12/#resourcerequirements-v1-core) | Describes the compute resource requirements for controller pod. <br/><sup>3</sup>_Deprecated and only available in `v1alpha1`._ |
| nodeResources | [ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.12/#resourcerequirements-v1-core) | Describes the compute resource requirements for the pods running on node(s). <br/>_<sup>3</sup>Deprecated and only available in `v1alpha1`._ |
...
<sup>3</sup> Pod level resource requirements (`nodeResources` and `controllerResources`)
...
$ kubectl get deployments.pmem-csi.intel.com/v1alpha1 pmem-csi.intel.com -o jsonpath='{.apiVersion}{"\n"}{.spec}{"\n"}'
# TODO: verify syntax and result
```
