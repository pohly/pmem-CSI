...
CRD_DIR=deploy/crd
CRD_KUSTOMIZE_DIR=deploy/kustomize/crd

KUBECONFIG := $(shell echo $(PWD)/_work/$(CLUSTER)/kube.config)

PATCH_CRD_ANNOTATION := sed -i -e 's;\(.*\)\.\(inject-ca-from.*\);\1/\2;g'
...
operator-generate-crd: controller-gen
	...
	@make --quiet operator-kustomize-crd
...
# Kustomize CRD by applying conversion webhook configuration patch
operator-kustomize-crd: _work/kustomize
	@_work/kustomize build --load_restrictor=none $(CRD_KUSTOMIZE_DIR) -o $(CRD_DIR)/pmem-csi.intel.com_deployments_webhook.yaml
	@$(PATCH_CRD_ANNOTATION) $(CRD_DIR)/pmem-csi.intel.com_deployments_webhook.yaml
