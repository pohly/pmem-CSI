...
var _ = Describe("Operator", func() {
	...
	Context("API", func() {
		...
		It("shall set defaults for empty deployment", func() {
			...
			Expect(d.Spec.ControllerDriverResources).ShouldNot(BeNil(), "default controller driver resources not set")
			rs := d.Spec.ControllerDriverResources.Requests
			Expect(rs.Cpu().String()).Should(BeEquivalentTo(base.DefaultControllerResourceRequestCPU), "controller driver 'cpu' resource request mismatch")
			Expect(rs.Memory().String()).Should(BeEquivalentTo(base.DefaultControllerResourceRequestMemory), "controller driver 'memory' resource request mismatch")
			...
			Expect(rs.Cpu().String()).Should(BeEquivalentTo(base.DefaultNodeResourceRequestCPU), "node driver 'cpu' resource request mismatch")
			Expect(rs.Memory().String()).Should(BeEquivalentTo(base.DefaultNodeResourceRequestMemory), "node driver 'cpu' resource request mismatch")
			rs = d.Spec.NodeDriverResources.Limits
			Expect(rs.Cpu().String()).Should(BeEquivalentTo(base.DefaultNodeResourceLimitCPU), "node driver 'cpu' resource limit mismatch")
			Expect(rs.Memory().String()).Should(BeEquivalentTo(base.DefaultNodeResourceLimitMemory), "node driver 'cpu' resource limit mismatch")
			...
			Expect(rs.Cpu().String()).Should(BeEquivalentTo(base.DefaultNodeRegistrarRequestCPU), "node registrar 'cpu' resource request mismatch")
			Expect(rs.Memory().String()).Should(BeEquivalentTo(base.DefaultNodeRegistrarRequestMemory), "node registrar 'cpu' resource request mismatch")
			rs = d.Spec.NodeRegistrarResources.Limits
			Expect(rs.Cpu().String()).Should(BeEquivalentTo(base.DefaultNodeRegistrarLimitCPU), "node registrar 'cpu' resource limit mismatch")
			Expect(rs.Memory().String()).Should(BeEquivalentTo(base.DefaultNodeRegistrarLimitMemory), "node registrar 'cpu' resource limit mismatch")
			...
			Expect(rs.Cpu().String()).Should(BeEquivalentTo(base.DefaultProvisionerRequestCPU), "provisioner 'cpu' resource request mismatch")
			Expect(rs.Memory().String()).Should(BeEquivalentTo(base.DefaultProvisionerRequestMemory), "provisioner 'cpu' resource request mismatch")
			rs = d.Spec.ProvisionerResources.Limits
			Expect(rs.Cpu().String()).Should(BeEquivalentTo(base.DefaultProvisionerLimitCPU), "provisioner 'cpu' resource limit mismatch")
			Expect(rs.Memory().String()).Should(BeEquivalentTo(base.DefaultProvisionerLimitMemory), "provisioner 'cpu' resource limit mismatch")
		})
